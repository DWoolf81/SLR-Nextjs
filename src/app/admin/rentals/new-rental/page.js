...
export default function RentalPage() {
  ...
  return (
    <>
      ...
      <div>
        <h2>
          New <RentalType type={state.value} />{" "}
        </h2>
      </div>
      {state.value == "camper" && (
        <Camper locations={locations} type={"insert"} />
      )}
      {state.value == "container" && (
        <Container locations={locations} type={"insert"} />
      )}
      {items.includes(state.value) && (
        <House locations={locations} type={"insert"} item={state.value} />
      )}
    </>
  );
}
